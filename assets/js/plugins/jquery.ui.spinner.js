...
$.widget('ui.spinner', {
    ...
    _createButtons: function(input) {
        ...
        if (input.filter(':visible').length == 0) {
            ...
            var copiedElem = this.element.clone()
                                          .attr("id", false)
                                          .css({visibility:"hidden", display:"block", position:"absolute"});

            $("body").append(copiedElem);
            var copiedElem_height = copiedElem.outerHeight();
            var copiedElem_width = copiedElem.width();
            copiedElem.remove();
        }
        ...
        var self = this,
            ...
            height = input.filter(':visible').length > 0 ? input.outerHeight() : copiedElem_height,
