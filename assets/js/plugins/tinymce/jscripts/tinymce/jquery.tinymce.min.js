        ...
        $.fn.tinymce = function(settings) {
                ...
                function init() {
                        ...
                        self.each(function(i, node) {
                                ...
                                if (settings.plugins && settings.pluginsAlias) {

                                    var baseUrl = settings.script_url.substring(0, settings.script_url.lastIndexOf("/")) + '/plugins/';

                                    var pluginArray = settings.plugins.split(" ");
                                    for (var idx in pluginArray) {
                                        if (settings.pluginsAlias[pluginArray[idx]]) {

                                            tinymce.PluginManager.load(pluginArray[idx], baseUrl + pluginArray[idx] + '/plugin.min.js');
                                            var preloadedPlugins = settings.pluginsAlias[pluginArray[idx]].split(" ");
                                            for (var pos in preloadedPlugins) {
                                                var pluginUri = baseUrl + preloadedPlugins[pos] + '/plugin.min.js';

                                                tinymce.PluginManager.load(preloadedPlugins[pos], pluginUri);
                                                tinymce.ScriptLoader.markDone(pluginUri);
                                            }

                                            settings.plugins = settings.plugins.replace(pluginArray[idx], settings.pluginsAlias[pluginArray[idx]]);
                                        }
